...
int main() {
    // Définir le nombre de threads pour le traitement parallèle en utilisant Eigen
    ...
    while (true) {
        ...
        std::cout << "Processus " << getpid() << " a reçu la tâche " << identifier << std::endl;
        ...
        cpr::Response post_response = cpr::Post(
            ...
            cpr::Body{result_json},
            cpr::Header{{"Content-Type", "application/json"}}
        );

        if (post_response.status_code == CODE_SUCCESS) {
            std::cout << "Résultat de la tâche " << identifier << " envoyé avec succès." << std::endl;
        } else {
            std::cerr << "Echec de l'envoi du résultat pour la tâche " << identifier
                      << ". Code HTTP : " << post_response.status_code << std::endl;
        }
